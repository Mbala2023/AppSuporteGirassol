...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const pedidosPendentes = mockOrders.filter((o) => o.status === "pendente").length
    const pedidosEmAndamento = mockOrders.filter((o) => o.status === "em_andamento" || o.status === "aceito").length
    const pedidosConcluidos = mockOrders.filter((o) => o.status === "concluido" || o.status === "avaliado").length
    const pedidosCancelados = mockOrders.filter((o) => o.status === "cancelado").length
    const totalClientes = mockUsers.filter((u) => u.role === "cliente").length
    const totalTecnicos = mockUsers.filter((u) => u.role === "tecnico" || u.role === "admin").length

    const somaAvaliacoes = mockRatings.reduce((acc, r) => acc + r.estrelas, 0)
    const avaliacaoMediaGeral = mockRatings.length > 0 ? somaAvaliacoes / mockRatings.length : 0
  ...
  const taxaConclusao = stats.totalPedidos > 0 ? ((stats.pedidosConcluidos / stats.totalPedidos) * 100).toFixed(1) : "0"

  const taxaCancelamento =
    stats.totalPedidos > 0 ? ((stats.pedidosCancelados / stats.totalPedidos) * 100).toFixed(1) : "0"

  const technicianStats = getTechnicianStatsByPeriod(mockUsers, mockOrders, mockRatings, selectedPeriod)
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground">Visão geral do sistema e estatísticas de atendimento</p>
        </div>
        ...
        <Tabs defaultValue="overview" className="space-y-6">
          ...
          <TabsContent value="overview" className="space-y-6">
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
              <Card>
                ...
                <CardContent>
                  ...
                  <p className="text-xs text-muted-foreground mt-1">Todos os pedidos registrados</p>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">{stats.pedidosPendentes + stats.pedidosEmAndamento}</div>
                  <p className="text-xs text-muted-foreground mt-1">Pendentes e em andamento</p>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent>
                  ...
                  <Progress value={Number.parseFloat(taxaConclusao)} className="mt-2" />
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">{stats.avaliacaoMediaGeral.toFixed(1)}</div>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="pt-4 border-t">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-2xl font-bold">{stats.totalClientes + stats.totalTecnicos}</div>
                    </div>
                  </div>
                </CardContent>
              </Card>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Indicadores de qualidade do serviço</CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-sm font-medium">Taxa de Conclusão</span>
                    <span className="text-sm text-muted-foreground">{taxaConclusao}%</span>
                  </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-sm text-muted-foreground">{taxaCancelamento}%</span>
                  </div>
                  <Progress value={Number.parseFloat(taxaCancelamento)} className="bg-red-100" />
                </div>
                <div>
                  <div className="flex items-center justify-between mb-2">
          ...
          <TabsContent value="rankings" className="space-y-6">
            ...
            <div className="grid gap-4 md:grid-cols-4">
              <Card>
                <CardHeader>
                  <CardTitle className="text-sm font-medium">Melhor Técnico</CardTitle>
                </CardHeader>
                <CardContent>
                  {technicianStats.length > 0 ? (
                    <div>
                      <p className="text-2xl font-bold">{technicianStats[0].nome}</p>
                      <p className="text-sm text-muted-foreground">{technicianStats[0].especialidade}</p>
                      <div className="flex items-center gap-2 mt-2">
                        <Badge variant="secondary">Score: {technicianStats[0].score}</Badge>
                        <div className="flex items-center gap-1">
                          <Star className="h-3 w-3 fill-yellow-400 text-yellow-400" />
                          <span className="text-sm font-medium">{technicianStats[0].avaliacaoMedia.toFixed(1)}</span>
                        </div>
                      </div>
                    </div>
                  ) : (
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-sm font-medium">Mais Atendimentos</CardTitle>
                </CardHeader>
                <CardContent>
                  {technicianStats.length > 0 ? (
                    <div>
                      <p className="text-2xl font-bold">
                        {[...technicianStats].sort((a, b) => b.totalAtendimentos - a.totalAtendimentos)[0].nome}
                      </p>
                      <p className="text-sm text-muted-foreground">
                        {
                          [...technicianStats].sort((a, b) => b.totalAtendimentos - a.totalAtendimentos)[0]
                            .totalAtendimentos
                        }{" "}
                        atendimentos
                      </p>
              ...
              <Card>
                ...
                <CardContent>
                  {technicianStats.filter((t) => (t.tempoMedioAtendimento || 0) > 0).length > 0 ? (
                    <div>
                      <p className="text-2xl font-bold">
                        {
                          [...technicianStats]
                            .filter((t) => (t.tempoMedioAtendimento || 0) > 0)
                            .sort((a, b) => (a.tempoMedioAtendimento || 0) - (b.tempoMedioAtendimento || 0))[0].nome
                        }
                      </p>
                      <div className="flex items-center gap-2 mt-2">
                        ...
                        <span className="text-lg font-bold">
                          {formatTime(
                            [...technicianStats]
                              .filter((t) => (t.tempoMedioAtendimento || 0) > 0)
                              .sort((a, b) => (a.tempoMedioAtendimento || 0) - (b.tempoMedioAtendimento || 0))[0]
              ...
              <Card>
                ...
                <CardContent>
                  {technicianStats.filter((t) => t.totalAvaliacoes > 0).length > 0 ? (
                    <div>
                      <p className="text-2xl font-bold">
                        {
                          [...technicianStats]
                            .filter((t) => t.totalAvaliacoes > 0)
                            .sort((a, b) => b.avaliacaoMedia - a.avaliacaoMedia)[0].nome
                        }
                      </p>
                      <div className="flex items-center gap-1 mt-2">
                        ...
                        <span className="text-lg font-bold">
                          {[...technicianStats]
                            .filter((t) => t.totalAvaliacoes > 0)
                            .sort((a, b) => b.avaliacaoMedia - a.avaliacaoMedia)[0]
          ...
          <TabsContent value="orders" className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Lista completa de pedidos no sistema</CardDescription>
              </CardHeader>
              <CardContent>
