...
export default function PedidosPage() {
  ...
  useEffect(() => {
    ...
    if (isClient) {
      ...
      setOrders(mockOrders.filter((o) => o.tecnicoId === user?.id || o.status === "pendente"))
    }
  ...
  const handleAcceptOrder = (orderId: string) => {
    ...
    if (user && order) {
      const welcomeMessage: ChatMessage = {
        ...
        mensagem: `Olá! Sou ${user.nome}, ${user.especialidade || "técnico"}. Aceitei seu pedido e estou pronto para atendê-lo. ${user.telefone ? `Você pode me contatar pelo telefone ${user.telefone}.` : ""} Vamos resolver seu problema!`,
        createdAt: new Date(),
        lida: false,
      }
    ...
    toast({
      title: "Pedido aceito",
      description: "Você aceitou este pedido. Uma mensagem automática foi enviada ao cliente.",
    })
  }
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-8">
        <h1 className="text-3xl font-bold mb-6">{isClient ? "Meus Pedidos" : "Atendimentos"}</h1>
        ...
        <Tabs defaultValue="ativos" className="space-y-6">
          <TabsList>
            ...
            {isTechnician && <TabsTrigger value="disponiveis">Disponíveis</TabsTrigger>}
          </TabsList>

          <TabsContent value="ativos" className="space-y-4">
            {filterOrdersByStatus(["pendente", "aceito", "em_andamento"]).length === 0 ? (
              <p className="text-center text-muted-foreground py-8">Nenhum pedido ativo no momento.</p>
            ) : (
              filterOrdersByStatus(["pendente", "aceito", "em_andamento"]).map((order) => (
                <OrderCard key={order.id} order={order} onCancel={handleCancelOrder} onComplete={handleCompleteOrder} />
              ))
            )}
          </TabsContent>
          ...
          <TabsContent value="concluidos" className="space-y-4">
            {filterOrdersByStatus(["concluido", "avaliado"]).length === 0 ? (
              <p className="text-center text-muted-foreground py-8">Nenhum pedido concluído.</p>
            ) : (
              filterOrdersByStatus(["concluido", "avaliado"]).map((order) => (
                <OrderCard key={order.id} order={order} onRate={handleRateOrder} />
              ))
            )}
          </TabsContent>
          ...
          <TabsContent value="cancelados" className="space-y-4">
            {filterOrdersByStatus(["cancelado"]).length === 0 ? (
              <p className="text-center text-muted-foreground py-8">Nenhum pedido cancelado.</p>
            ) : (
              filterOrdersByStatus(["cancelado"]).map((order) => <OrderCard key={order.id} order={order} />)
            )}
          </TabsContent>

          {isTechnician && (
            <TabsContent value="disponiveis" className="space-y-4">
              {orders.filter((o) => o.status === "pendente" && !o.tecnicoId).length === 0 ? (
                <p className="text-center text-muted-foreground py-8">Nenhum pedido disponível no momento.</p>
              ) : (
                orders
                  .filter((o) => o.status === "pendente" && !o.tecnicoId)
                  .map((order) => <OrderCard key={order.id} order={order} onAccept={handleAcceptOrder} />)
              )}
            </TabsContent>
          )}
        </Tabs>
      ...
      {orderToRate && (
        <RatingDialog
          ...
          technicianName={getUserById(orderToRate.tecnicoId || "")?.nome || "Técnico"}
        />
      )}
    </div>
  )
